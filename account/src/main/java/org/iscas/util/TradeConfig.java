 ...
 * TradeConfig sets runtime parameters such as the RunTimeMode
 * 交易相关的配置信息，在DayTrader应用中的配置和设置的情况等。
 */
...
public class TradeConfig {
   ...
   //Disable Web JDBC&JPA public static String[] runTimeModeNames = {"Full EJB3", "Direct (JDBC)", "Session (EJB3) To Direct", "Web JDBC", "Web JPA"};    
    ...
    /* 以上是连接模式的设置，EJB, DIRECT, SESSION3， JDBC, JPA 5种连接方式；运行时使用的方式默认是直连。 */

    
    /* Trade JPA Layer parameters 使用JPA时的一些参数*/
    ...
    /* 订单处理的模式；同步处理synchronous，异步两阶段处理 asynchronous-2-phase */
    ...
    /*访问模式的设置：标准访问模式和以WebService方式访问*/
    ...
    /* Trade 交易场景下的负载参数设置，混合负载：标准，高负载High-Volume*/
    ...
    /* 股票交易数据库的特定参数： JDBC的用户名和密码；使用数据源Datasource java:comp/env/jdbc/TradeDataSource */
    ...
    private static char actions[] =
        { 'h', 'q', 'l', 'o', 'r', 'a', 'p', 'b', 's', 'u' };
    private static int sellDeficit = 0; //卖出亏损股票；deficit 亏损，亏空；赤字
    //Tracks the number of buys over sell when a users portfolio is empty 业务量为空； portfolio 业务量
