...
public class KeySequenceDirect {
    ...
    private static final String getKeyForUpdateSQL = "select * from keygen kg where kg.keyname = ?  for update";

    private static final String createKeySQL =
        "insert into keygen " + "( keyname, keyval ) " + "VALUES (  ?  ,  ? )";

    private static final String updateKeyValueSQL = "update keygen set keyval = ? " + "where keyname = ?";
