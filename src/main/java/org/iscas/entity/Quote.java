...
//    @NamedQuery(name = "quote.quotesByChange",query = "SELECT q FROM quote q WHERE q.symbol LIKE 's:1__' ORDER BY q.change1 DESC"),
    @NamedQuery(name = "quote.quotesByChange",query = "SELECT q FROM quote q WHERE q.symbol LIKE 's:1__' ORDER BY q.change1 "),
    @NamedQuery(name = "quote.findByLow", query = "SELECT q FROM quote q WHERE q.low = :low"),
    @NamedQuery(name = "quote.findByOpen1", query = "SELECT q FROM quote q WHERE q.open1 = :open1"),
    @NamedQuery(name = "quote.findByVolume", query = "SELECT q FROM quote q WHERE q.volume = :volume"),
    @NamedQuery(name = "quote.findByPrice", query = "SELECT q FROM quote q WHERE q.price = :price"),
    @NamedQuery(name = "quote.findByHigh", query = "SELECT q FROM quote q WHERE q.high = :high"),
    @NamedQuery(name = "quote.findByCompanyname", query = "SELECT q FROM quote q WHERE q.companyName = :companyname"),
    @NamedQuery(name = "quote.findBySymbol", query = "SELECT q FROM quote q WHERE q.symbol = :symbol"),
    @NamedQuery(name = "quote.findByChange1", query = "SELECT q FROM quote q WHERE q.change1 = :change1")
})
@NamedNativeQueries({
    @NamedNativeQuery(name="quote.quoteForUpdate", query="select * from quote q where q.symbol=? for update",resultClass=org.iscas.entity.Quote.class)
})
